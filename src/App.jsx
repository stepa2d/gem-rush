...
const COL=[{f:"#E53935",l:"#FF8A80",d:"#B71C1C",n:"cherry"},{f:"#FF9800",l:"#FFE0B2",d:"#E65100",n:"orange"},{f:"#FDD835",l:"#FFF9C4",d:"#F9A825",n:"lemon"},{f:"#AB47BC",l:"#E1BEE7",d:"#6A1B9A",n:"grape"},{f:"#1E88E5",l:"#90CAF9",d:"#0D47A1",n:"seven"},{f:"#EC407A",l:"#F8BBD0",d:"#AD1457",n:"bell"}];
...
function drawFruit(ctx,fi,cx,cy,sz){
  ...
  else if(fc.n==="orange"){ctx.fillStyle=gl(cx,cy,sz,"#FFE082","#FF9800","#E65100");ctx.beginPath();ctx.arc(cx,cy,sz,0,Math.PI*2);ctx.fill();ctx.strokeStyle="rgba(255,224,130,0.3)";ctx.lineWidth=1.5;for(let i=0;i<8;i++){const a=i*Math.PI/4;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+Math.cos(a)*sz*.75,cy+Math.sin(a)*sz*.75);ctx.stroke()}ctx.fillStyle="#66BB6A";ctx.beginPath();ctx.ellipse(cx,cy-sz*.85,sz*.18,sz*.1,0,0,Math.PI*2);ctx.fill();sh(cx,cy,sz);sd(cx,cy,sz)}
